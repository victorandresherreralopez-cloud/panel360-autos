    ...
    >
      <span className="logo-envoltura inline-flex shrink-0 items-center">
        <Image
          ...
          className={clsx("logo-imagen h-auto object-contain", logoSizes[variant])}
        />
      </span>
      ...
      {showModule ? (
        <span
          className={clsx(
            "min-w-0 border-l border-graphite/14 pl-3 leading-tight",
            isMobile && "pl-2",
            isAuth && "pl-4"
          )}
        >
          <span className={clsx("block font-black uppercase text-ink", isMobile ? "text-[10px]" : "text-xs")}>
            Autos
          </span>
          {!isMobile ? (
            <span className={clsx("block font-semibold text-steel", isAuth ? "text-sm" : "text-[11px]")}>
